...
const EditAlbum = () => {
  ...
  const uploadNFTToS3Bucket = async (e: any) => {
    ...
    try {
      setIsLoading(true);
      let fileImageCid = currentAlbumMetaData ? currentAlbumMetaData.image : "";
      if (isImageChanged) {
      ...
      if (tempCurrentMetaId) {
        ...
        const storageAlbums = storageAlbumsData
          ? JSON.parse(storageAlbumsData)
          : [];
        storageAlbums[currentId] = {
          id: storageAlbums[currentId].id,
          metadata: tempCurrentMetaId,
          songs: storageAlbums[currentId].songs,
        };
        localStorage.setItem("albums", JSON.stringify(storageAlbums));
        toastFunction(`Album updated on ${tempCurrentMetaId}`);
        emptyFields();
        setIsLoading(false);
      } else {
        setIsLoading(false);
  ...
  const uploadSoundNFTToS3Bucket = async (e: any) => {
    ...
    try {
      ...
      if (tempCurrentMetaId) {
        ...
        if (storageAlbums.length && storageAlbums[currentId]) {
          const storageSongs = storageAlbums[currentId].songs
            ? storageAlbums[currentId].songs
            : [];

          storageSongs.push(tempCurrentMetaId);
          storageAlbums[currentId].songs = storageSongs;
  ...
  const fetchSongsFromStorage = async () => {
    ...
    if (storageAlbums.length && storageAlbums[currentId]) {
      const albumMetaData = (
        await Promise.all(
          storageAlbums[currentId].songs.map(async (song: string) => {
            const axiosConfig = {
              method: "get",
              url: `https://ipfs.io/ipfs/${song}`,
              headers: {
  ...
  const removeSongs = async (id: number) => {
    ...
    storageAlbums[currentId].songs.splice(id, 1);
    localStorage.setItem("albums", JSON.stringify(storageAlbums));
    await fetchSongsFromStorage();
