...
function lookupArchive(network, opts) {
    ...
    if (!opts.type) {
        let isSubstrateNetwork = registry_1.archivesRegistrySubstrate.archives.some((a) => a.network === network);
        let isEvmNetwork = registry_1.archivesRegistryEVM.archives.some((a) => a.network === network);
...
Please consider submitting a PR to subsquid/archive-registry github repo to extend the registry`);
    ...
    switch (opts?.type) {
        case 'Substrate':
            return lookupInSubstrateRegistry(network, registry_1.archivesRegistrySubstrate, opts)[0].dataSourceUrl;
        case 'EVM':
            return lookupInEVMRegistry(network, registry_1.archivesRegistryEVM, opts)[0].dataSourceUrl;
        default:
...
function lookupInEVMRegistry(network, registry, opts) {
    let archives = registry_1.archivesRegistryEVM.archives.filter((a) => a.network.toLowerCase() === network.toLowerCase());
    if (archives.length > 1) {
        throw new Error(`There are multiple networks with name ${network}. \
Provide the genesis hash to disambiguate.`);
    }
...
function getChainInfo(network, genesis) {
    let matched = registry_1.networkRegistrySubstrate.networks.filter((n) => n.name.toLowerCase() === network.toLowerCase());
    if (genesis) {
