...
type CallFields<R extends CallRequest> = Select<CallScalars, R> & (R['parent'] extends true ? {
    parent?: CallFields<R>;
} : R['parent'] extends PlainReq<SubstrateCall> ? {
    parent?: CallFields<R['parent']>;
} : {});
export type CallType<R> = R extends true ? SubstrateCall : R extends CallRequest ? CallFields<R> : never;
type ExtrinsicFields<R extends ExtrinsicRequest> = Select<ExtrinsicScalars, R> & (R['call'] extends true ? {
    call: SubstrateCall;
} : R['call'] extends CallRequest ? {
    call: CallFields<R['call']>;
} : {});
export type ExtrinsicType<R> = R extends true ? SubstrateExtrinsic : R extends ExtrinsicRequest ? ExtrinsicFields<R> : never;
...
type EventFields<R extends EventRequest, E extends SubstrateEvent = SubstrateEvent> = E extends any ? Select<EventScalars<E>, R> & AddOption<WithProp<'call', AddUndefined<E['call'], CallType<R>>>> & AddOption<WithProp<'extrinsic', AddUndefined<E['extrinsic'], ExtrinsicType<R['extrinsic']>>>> : never;
type GenericEventType<R, E extends SubstrateEvent = SubstrateEvent> = R extends true ? E : R extends EventRequest ? EventFields<R, E> : never;
export type EventType<R, N = string> = N extends 'Contracts.ContractEmitted' ? GenericEventType<R, ContractsContractEmittedEvent> : N extends 'EVM.Log' ? GenericEventType<R, EvmLogEvent> : N extends 'Gear.MessageEnqueued' ? GenericEventType<R, GearMessageEnqueuedEvent> : N extends 'Gear.UserMessageSent' ? GenericEventType<R, GearUserMessageSentEvent> : GenericEventType<R>;
export interface EventDataRequest {
    event?: boolean | EventRequest;
}
export type EventData<R extends EventDataRequest = {
    event: true;
...
export type CallData<R extends CallDataRequest = {
    call: true;
    extrinsic: true;
}> = WithProp<"call", CallType<R["call"]>> & WithProp<"extrinsic", ExtrinsicType<R["extrinsic"]>>;
type SetName<T, N> = T extends any ? Omit<T, "name"> & {
    name: N;
} : never;
type SetItemName<T, P, N> = P extends keyof T ? Omit<T, P> & {
...
export type EventItem<N, R = false> = WithKind<"event", SetItemName<R extends true ? EventData<{
    event: true;
}, N> : R extends EventDataRequest ? EventData<R, N> : EventData<{
    event: {};
}, N>, 'event', N>>;
export type CallItem<Name, R = false> = WithKind<"call", SetItemName<R extends true ? CallData : R extends CallDataRequest ? CallData<R> : CallData<{
    call: {};
}>, "call", Name>>;
...
type AddItem<T extends ItemKind, I extends ItemKind, R> = (T extends Pick<I, "kind" | "name"> ? ItemMerge<T, I, R> : T) | Exclude<I, Pick<T, "kind" | "name">>;
