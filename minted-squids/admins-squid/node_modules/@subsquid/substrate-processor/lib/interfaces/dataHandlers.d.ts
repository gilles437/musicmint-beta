...
type BlockEventItem<R> = R extends true ? EventItem<string, true> : R extends BlockEventsRequest ? ["*"] extends [keyof R] ? [keyof R] extends ["*"] ? EventItem<string, R["*"]> : {
...
type BlockCallItem<R> = R extends true ? CallItem<string, true> : R extends BlockCallsRequest ? ["*"] extends [keyof R] ? [keyof R] extends ["*"] ? CallItem<string, R["*"]> : {
...
type BlockItem<R> = R extends true ? BlockEventItem<true> | BlockCallItem<true> : R extends BlockItemRequest ? BlockEventItem<R['events']> | BlockCallItem<['calls']> : BlockEventItem<false> | BlockCallItem<false>;
...
export type EventHandlerContext<S, R extends EventDataRequest = {
    event: true;
...
export type CallHandlerContext<S, R extends CallDataRequest = {
    call: true;
    extrinsic: true;
...
export type EvmLogHandlerContext<S, R extends EventDataRequest = {
    event: true;
...
export type ContractsContractEmittedHandlerContext<S, R extends EventDataRequest = {
    event: true;
...
export type GearMessageEnqueuedHandlerContext<S, R extends EventDataRequest = {
    event: true;
...
export type GearUserMessageSentHandlerContext<S, R extends EventDataRequest = {
    event: true;
